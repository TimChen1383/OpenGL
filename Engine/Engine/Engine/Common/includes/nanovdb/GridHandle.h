...
void GridHandle<BufferT>::read(std::istream& is, const BufferT& pool)
{
    ...
    if (data.isValid()) {
        ...
        is.seekg(-static_cast<std::streamoff>(sizeof(GridData)), std::ios::cur);// rewind to start
        is.read((char*)(buffer.data()), buffer.size());
        *this = GridHandle(std::move(buffer));
    } else {
        is.seekg(-static_cast<std::streamoff>(sizeof(GridData)), std::ios::cur);// rewind
...
void GridHandle<BufferT>::read(std::istream& is, uint32_t n, const BufferT& pool)
{
    ...
    if (data.isValid()) {
        ...
        *this = GridHandle(std::move(buffer));
        ...
        is.seekg(-static_cast<std::streamoff>(sizeof(GridData)), std::ios::cur);// rewind sizeof(GridData) bytes to undo initial read
...
void GridHandle<BufferT>::read(std::istream& is, const std::string &gridName, const BufferT& pool)
{
    ...
    is.seekg(-static_cast<std::streamoff>(byteSize), std::ios::cur);// rewind
    if (data.isValid()) {
        uint32_t n = 0;
