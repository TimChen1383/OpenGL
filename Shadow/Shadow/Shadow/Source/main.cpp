...
bool debugShadowMap = false; // Toggle for debug shadow map visualization
bool directionalLightEnabled = false; // Toggle for directional light (disabled by default for debugging)

// Directional light rotation
float dirLightAngle = 0.0f;
float dirLightRotateSpeed = 20.0f;
...
int main()
{
	...
	std::cout << "Loading shadow mapping shaders..." << std::endl;
	ShadowMapShader.loadShaders("ShadowMap.vert", "ShadowMap.frag");
	std::cout << "Shadow mapping shaders loaded." << std::endl;
	
	// Debug: Print shadow mapping setup
	...
	while (!glfwWindowShouldClose(gwindow))
	{
		...
		// Flashlight properties - Stronger spotlight intensity
		...
		float spotLightIntensity = flashlightEnabled ? 80.0f : 0.0f; // MUCH higher intensity
		glm::vec3 spotLightColor = glm::vec3(1.0f, 0.95f, 0.8f) * spotLightIntensity; // Warm white color

		// --- Directional light parameters ---
        // Animate the directional light direction (rotating around Y axis)
        dirLightAngle += (float)deltaTime * dirLightRotateSpeed;
        float dirX = sinf(glm::radians(dirLightAngle));
        float dirZ = cosf(glm::radians(dirLightAngle));
        glm::vec3 dirLightDirection = glm::normalize(glm::vec3(dirX, -1.0f, dirZ));
        glm::vec3 dirLightColor = directionalLightEnabled ? glm::vec3(0.8f, 0.8f, 0.7f) : glm::vec3(0.0f, 0.0f, 0.0f); // Toggle directional light
        
        // Calculate directional light space matrix for shadow mapping
        float dirNear = 1.0f, dirFar = 20.0f;
        float dirSize = 15.0f; // Orthographic projection size
        glm::mat4 dirLightProjection = glm::ortho(-dirSize, dirSize, -dirSize, dirSize, dirNear, dirFar);
        glm::vec3 dirLightPos = -dirLightDirection * 10.0f; // Position light far away in opposite direction
        glm::mat4 dirLightView = glm::lookAt(dirLightPos, glm::vec3(0.0f), glm::vec3(0.0, 1.0, 0.0));
        glm::mat4 dirLightSpaceMatrix = dirLightProjection * dirLightView;

		// Calculate light space matrix for shadow mapping
		float near_plane = 0.5f, far_plane = 50.0f;
		// Use the same FOV as the spotlight cone (slightly wider for shadow coverage)
		float lightFOV = glm::degrees(acos(spotLightOuterCutoff)) * 2.0f + 10.0f; // Add 10 degrees padding
		glm::mat4 lightProjection = glm::perspective(glm::radians(lightFOV), 1.0f, near_plane, far_plane);
		glm::mat4 lightView = glm::lookAt(spotLightPos, spotLightPos + spotLightDir, glm::vec3(0.0, 1.0, 0.0));
		glm::mat4 lightSpaceMatrix = lightProjection * lightView;

		// 1. SHADOW PASS: Render depth of scene to texture
		glViewport(0, 0, SHADOW_WIDTH, SHADOW_HEIGHT);
		glBindFramebuffer(GL_FRAMEBUFFER, depthMapFBO);
		glClear(GL_DEPTH_BUFFER_BIT);
		
		// Only render if flashlight is enabled
		if (flashlightEnabled) {
			ShadowMapShader.use();
			ShadowMapShader.setUniform("lightSpaceMatrix", lightSpaceMatrix);
			
			// Render models to shadow map (objects that cast shadows)
			...
			// DON'T render ground plane to shadow map - it should receive shadows, not cast them
		}
		
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		
		// 1.5. DIRECTIONAL LIGHT SHADOW PASS: Render depth of scene from directional light perspective
		glViewport(0, 0, SHADOW_WIDTH, SHADOW_HEIGHT);
		glBindFramebuffer(GL_FRAMEBUFFER, dirDepthMapFBO);
		glClear(GL_DEPTH_BUFFER_BIT);
		
		// Always render directional light shadows
		ShadowMapShader.use();
		ShadowMapShader.setUniform("lightSpaceMatrix", dirLightSpaceMatrix);
		
		// Render models to directional light shadow map
		for (int i = 0; i < numModels; i++)
		{
			model = glm::mat4(1.0f);
			model = glm::scale(glm::mat4(1.0f), modelScale[i]) * glm::translate(glm::mat4(1.0f), modelPos[i]);
		...
		// 2. REGULAR RENDER PASS: Render scene as normal using the generated depth/shadow map
		glViewport(0, 0, gWindowWidth, gWindowHeight);
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

		// Configure shader uniforms for lighting pass
		LightingShader.use();
		...
		glBindTexture(GL_TEXTURE_2D, dirDepthMap);
		LightingShader.setUniform("dirShadowMap", 2);
		...
		GroundShader.setUniform("lightSpaceMatrix", lightSpaceMatrix);
		...
		GroundShader.setUniform("dirLightDirection", dirLightDirection);
        GroundShader.setUniform("dirLightColor", dirLightColor);
        GroundShader.setUniform("dirLightSpaceMatrix", dirLightSpaceMatrix);

		// Bind shadow map textures for ground shader
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, depthMap);
		...
		glActiveTexture(GL_TEXTURE2);
		glBindTexture(GL_TEXTURE_2D, dirDepthMap);
		GroundShader.setUniform("dirShadowMap", 2);
